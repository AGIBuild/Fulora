...
{
    private const string BridgeDistributionInvariantId = "GOV-040";
                ...
                async () =>
                {
                    ...
                    catch (Exception ex)
                    {
                        failures.Add(new GovernanceFailure(
                            Category: "bridge-distribution",
                            InvariantId: BridgeDistributionInvariantId,
                            SourceArtifact: "packages/bridge",
                            Expected: "npm install+build succeeds",
                            Actual: ex.Message));
                        ...
                        try
                        {
                            ...
                            if (!passed)
                                failures.Add(new GovernanceFailure(
                                    Category: "bridge-distribution",
                                    InvariantId: BridgeDistributionInvariantId,
                                    SourceArtifact: $"{pm} consume-smoke",
                                    Expected: "SMOKE_PASSED in output",
                                    Actual: output));
                        }
                        catch (Exception ex)
                        {
                        ...
                        try
                        {
                            ...
                            if (!passed)
                                failures.Add(new GovernanceFailure(
                                    Category: "bridge-distribution",
                                    InvariantId: BridgeDistributionInvariantId,
                                    SourceArtifact: $"node-lts-import ({nodeVersion})",
                                    Expected: "LTS_IMPORT_OK in output",
                                    Actual: importCheck));
                        }
                        catch (Exception ex)
                        {
                            failures.Add(new GovernanceFailure(
                                ...
                                InvariantId: BridgeDistributionInvariantId,
                                SourceArtifact: $"node-lts-import ({nodeVersion})",
                                Expected: "LTS import check passes",
                                Actual: ex.Message));
                    ...
                    else
                    {
                        failures.Add(new GovernanceFailure(
                            Category: "bridge-distribution",
                            InvariantId: BridgeDistributionInvariantId,
                            SourceArtifact: distIndex.ToString(),
                            Expected: "dist/index.js exists after build",
                            Actual: "missing"));
    ...
    internal Target DistributionReadinessGovernance => _ => _
        ...
        {
            RunGovernanceCheck(
                "Distribution readiness governance",
    ...
    private static void EvaluateAdoptionReadiness(string laneContext, string producerTarget, string expectedRuntimeLaneContext)
    {
        RunGovernanceCheck(
            ...
            () =>
            {
                ...
                return new GovernanceCheckResult(blockingFindings, reportPayload);
            });
    }
}
