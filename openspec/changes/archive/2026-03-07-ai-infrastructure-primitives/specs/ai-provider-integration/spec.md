...
- **THEN** the system invokes `IChatClient.CompleteAsync` on the resolved provider and returns the response through the bridge

#### Scenario: JS calls streaming chat completion
- **WHEN** JS calls `aiChat.completeStreaming(messages)` via bridge
- **THEN** the system invokes `IChatClient.CompleteStreamingAsync` and streams `IAsyncEnumerable` chunks to JS as `AsyncIterable`
